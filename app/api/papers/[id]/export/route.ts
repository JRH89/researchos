...
async function paper(id: string) { const db = new Pool({ connectionString: process.env.DATABASE_URL }); try { const r = await db.query("SELECT title, content_markdown FROM workspace_papers WHERE id = $1", [id]); return r.rows[0]; } finally { await db.end(); } }
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const item = await paper((await params).id); if (!item) return NextResponse.json({ error: "Paper not found." }, { status: 404 });
