...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (format === "docx") { const pages = item.content_markdown.split("\n---\n"); const children = pages.flatMap((page: string, pageIndex: number) => [...(pageIndex ? [new Paragraph({ children: [new PageBreak()] })] : []), ...page.split("\n").map((text: string) => new Paragraph({ children: [new TextRun(text)] }))]); const document = new Document({ sections: [{ children }] }); return new NextResponse(new Uint8Array(await Packer.toBuffer(document)), { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "Content-Disposition": `attachment; filename="${filename}.docx"` } }); }
