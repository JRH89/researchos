...
async function paper(id: string, ownerUid: string) { const db = new Pool({ connectionString: process.env.DATABASE_URL }); try { const r = await db.query("SELECT title, content_markdown, citation_style, paper_metadata FROM workspace_papers WHERE id = $1 AND owner_uid = $2", [id, ownerUid]); return r.rows[0]; } finally { await db.end(); } }
