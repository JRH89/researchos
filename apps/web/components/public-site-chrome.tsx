...
export function SiteHeader({ signedIn = false, credits, onSignIn, onBilling, onSignOut }: SiteHeaderProps) {
  ...
  return <><header className="app-header"><div className="app-header-inner">
    ...
    <nav className={`${open ? "public-nav open" : "public-nav"}${signedIn ? " signed-nav" : ""}`} aria-label="Site navigation">
      {!signedIn && <><a href="/#features" onClick={close}>Features</a><a href="/#how-it-works" onClick={close}>How it works</a><a href="/#pricing" onClick={close}>Pricing</a><a href="/blog" onClick={close}>Blog</a><a href="/#about" onClick={close}>About</a><a href="/#faq" onClick={close}>FAQ</a><a href="/support" onClick={close}>Support</a></>}
