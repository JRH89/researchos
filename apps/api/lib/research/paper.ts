...
import { getPaper, savePaper, workspaceSessions, type CitationStyle, type PaperMetadata, type WorkspacePaper } from "./store";
import type { Evidence } from "./contracts";

type PaperInput = { sessionIds?: unknown; evidenceIds?: unknown; title?: unknown; citationStyle?: unknown; targetWordCount?: unknown; sourceLimit?: unknown; paperType?: unknown; metadata?: unknown; revisionBrief?: unknown };
...
export async function draftWorkspacePaper(workspaceId: string, ownerUid: string, input: PaperInput, progress: Progress = () => {}, parentPaper?: WorkspacePaper) : Promise<WorkspacePaper> {
  ...
  const title = typeof input.title === "string" && input.title.trim() ? input.title.trim() : parentPaper?.title || "Workspace research paper";
  const citationStyle: CitationStyle = input.citationStyle === "MLA" ? "MLA" : parentPaper?.citationStyle || "APA";
  const targetWordCount = Math.max(250, Math.min(10000, typeof input.targetWordCount === "number" ? input.targetWordCount : parentPaper?.targetWordCount || 1000));
  const rawMetadata = typeof input.metadata === "object" && input.metadata ? input.metadata as Record<string, unknown> : {};
  const paperType = input.paperType === "essay" ? "essay" : parentPaper?.paperMetadata.paperType === "essay" ? "essay" : "research-paper";
  ...
  const body = (await writePaperBody({ title, paperType, citationStyle, targetWordCount, researchQuestions: selected.map((session) => session.question), evidence, priorDraft: parentPaper?.contentMarkdown, revisionBrief: typeof input.revisionBrief === "string" ? input.revisionBrief.trim() : undefined })).replace(new RegExp(`^\\s*(?:#\\s*)?${titlePattern}\\s*\\n+`, "i"), "").trim();
  ...
  return savePaper(ownerUid, { workspaceId, title, contentMarkdown: markdown, citations: evidence, sourceSessionIds: selected.map((session) => session.id), citationStyle, targetWordCount, paperMetadata: metadata, parentPaperId: parentPaper?.id });
}

export async function reviseWorkspacePaper(paperId: string, ownerUid: string, input: PaperInput, progress: Progress = () => {}) {
  const priorPaper = await getPaper(paperId, ownerUid);
  if (!priorPaper) throw new Error("The saved paper was not found.");
  return draftWorkspacePaper(priorPaper.workspaceId, ownerUid, { ...input, sessionIds: input.sessionIds ?? priorPaper.sourceSessionIds }, progress, priorPaper);
}
