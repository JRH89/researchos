...
function client() { const apiKey = runtimeEnv("OPENAI_API_KEY"); return apiKey ? new OpenAI({ apiKey, baseURL: runtimeEnv("OPENAI_BASE_URL") || undefined }) : undefined; }
function claude() { const apiKey = runtimeEnv("ANTHROPIC_API_KEY"); return apiKey ? new Anthropic({ apiKey }) : undefined; }
...
async function structured<T>(name: string, schema: z.ZodType<T>, prompt: string): Promise<T | undefined> {
  ...
  const response = await api.responses.create({ model: runtimeEnv("OPENAI_MODEL") || "gpt-5-mini", input: governedPrompt, text: { format: { type: "json_schema", name, strict: true, schema: responseSchema(name) } } });
...
export async function writePaperBody(input: { title: string; paperType: "research-paper" | "essay"; citationStyle: "APA" | "MLA"; targetWordCount: number; researchQuestions?: string[]; evidence: Evidence[]; priorDraft?: string; revisionBrief?: string }): Promise<string> {
  ...
  const revisionRule = input.priorDraft ? `This is a revision of an existing student draft. Preserve its central thesis, successful organization, and supported reasoning where possible. Expand and improve it into one coherent paper instead of restarting from an unrelated outline. Incorporate the selected evidence and correct unsupported statements. ${input.revisionBrief ? `Revision request: ${input.revisionBrief}` : "Revision request: deepen the analysis and reach the new target length."}\n\nExisting draft:\n${input.priorDraft.slice(0, 24000)}\n\n` : "";
...
${revisionRule}Selected research questions: ${input.researchQuestions?.join(" | ") || input.title}

Sources:\n${sources}`;
  const anthropic = claude();
  if (anthropic) {
    const response = await anthropic.messages.create({ model: runtimeEnv("ANTHROPIC_MODEL") || "claude-sonnet-4-5-20250929", max_tokens: Math.min(8000, Math.max(1600, Math.ceil(input.targetWordCount * 2))), messages: [{ role: "user", content: prompt }] });
    const text = response.content.filter((block) => block.type === "text").map((block) => block.text).join("\n").trim();
    if (text) return removeEmDashes(text);
  }
  const api = client();
  if (api) { const response = await api.responses.create({ model: runtimeEnv("OPENAI_MODEL") || "gpt-5-mini", input: prompt }); if (response.output_text.trim()) return removeEmDashes(response.output_text.trim()); }
