...
function developmentBypass() {
  return runtimeEnv("NODE_ENV") !== "production" && runtimeEnv("RESEARCHOS_AUTH_MODE") === "disabled";
}

async function adminAuth() {
  const configuredPath = runtimeEnv("FIREBASE_SERVICE_ACCOUNT_PATH");
  const repositoryRoot = existsSync(resolve(process.cwd(), ".env")) ? process.cwd() : resolve(process.cwd(), "../..");
  const raw = runtimeEnv("FIREBASE_SERVICE_ACCOUNT_JSON") || (configuredPath ? await readFile(resolve(configuredPath.startsWith("/") || /^[A-Za-z]:[\\/]/.test(configuredPath) ? "" : repositoryRoot, configuredPath), "utf8") : undefined);
