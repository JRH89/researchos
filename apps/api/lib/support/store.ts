...
export async function createTicket(ownerUid: string | undefined, email: string, subject: string, message: string) { const result = await database().query("INSERT INTO support_tickets (id, owner_uid, email, subject, message) VALUES ($1,$2,$3,$4,$5) RETURNING *", [crypto.randomUUID(), ownerUid || null, email, subject, message]); return map(result.rows[0]); }
