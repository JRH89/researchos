...
class StdioMcpTransport implements MCPTransport {
  ...
  private async connect() {
    ...
    const apiRoot = existsSync(resolve(process.cwd(), "apps/api/mcp")) ? resolve(process.cwd(), "apps/api") : process.cwd();
    const args = (this.config.args ?? []).map((arg) => arg.startsWith("mcp/") ? resolve(apiRoot, arg) : arg);
    this.transport = new StdioClientTransport({ command: this.config.command, args, env: { ...inheritedEnvironment, ...this.config.env } });
...
function configuredServerConfigs(): ServerConfig[] {
  const raw = runtimeEnv("MCP_SERVERS_JSON");
  if (!raw) return [];
...
export function configuredTransports(): MCPTransport[] {
  ...
  if (runtimeEnv("RESEARCHOS_DEMO_MODE") === "true") return [new DemoTransport()];
