...
export function ResearchWorkbench() {
  ...
  async function draftPaper() {
    ...
    try {
      const response = await fetch(api(`/api/workspaces/${workspaceId}/papers/stream`), { method: "POST", headers: authenticatedHeaders(), body: JSON.stringify({ sessionIds: selectedPaperRunIds, title: paperTitle, citationStyle, targetWordCount, paperType, metadata: paperMetadata }) });
  ...
  return <div className="shell">
    ...
    {workspaceId && workspaceRuns.length > 0 && <section className="paper-run-selector"><div><p className="eyebrow">Paper sources</p><strong>Select the research runs to include</strong></div><div className="paper-run-options">{workspaceRuns.map((run) => <label key={run.id}><input type="checkbox" checked={selectedPaperRunIds.includes(run.id)} onChange={(event) => setSelectedPaperRunIds((current) => event.target.checked ? [...current, run.id] : current.filter((id) => id !== run.id))} />{run.question}</label>)}</div></section>}
    ...
    {session && <section className="paper-actions"><button onClick={draftPaper} disabled={!workspaceId || paperWriting}>{paperWriting ? "Drafting paper…" : "Draft cited paper from workspace"}</button><small>{workspaceId ? "Combines evidence from all saved sessions in this workspace." : "Choose a workspace to enable paper drafting."}</small></section>}
